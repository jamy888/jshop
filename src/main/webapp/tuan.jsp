...
background: url(${basepath}<%=SystemManager.getInstance().getSystemSetting().getDefaultProductImg()%>) no-repeat 50% 50%;
<%}%>
}
.lazy {
  display: none;
...
function defaultProductImg(){ 
	...
	img.src="${basepath}<%=SystemManager.getInstance().getSystemSetting().getDefaultProductImg()%>"; 
